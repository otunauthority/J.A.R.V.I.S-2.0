...
(() => {
  ...
  function getResponse(input) {
    ...
    if (/select (media )?files?|pick (media )?files?|choose (media )?files?|add (media )?files?/.test(q)) {
  ...
  function initCardDrag() {
    ...
    // data-cmd buttons handled globally in bindEvents
  }
  ...
  function indexFileList(fileList, label) {
    ...
    if (files.length === 0) {
      ...
      typewriter('Indexed ' + files.length + ' media file' + (files.length === 1 ? '' : 's') + '. Tap a file to play, or say play music / play track 1.');
    }
  }
