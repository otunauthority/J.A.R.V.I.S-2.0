...
(() => {
  ...
  function getResponse(input) {
    ...
    if (/check battery|battery (level|status)|how much battery/.test(q)) {
      ...
      if (navigator.getBattery) {
        return navigator.getBattery().then(bat => {
          ...
          const ch = bat.charging ? 'charging' : 'discharging';
          cachedBattery = { level: bat.level, charging: bat.charging };
          return 'Battery is at ' + pct + '% and is ' + ch + ', Epidexios. Power matrix is on screen.';
    ...
    if (/power|reactor|energy|arc\b|battery level|power levels?/.test(q)) {
      ...
      const extra = bat.source === 'device'
        ? ' Device battery reads ' + bat.pct + ' percent' + (bat.charging ? ' and is charging.' : '.')
        : '';
      return pick(responses.power) + extra;
    }
  ...
  function initBattery() {
    ...
    navigator.getBattery().then(bat => {
      const sync = () => {
        cachedBattery = { level: bat.level, charging: bat.charging };
        // Keep top-bar power metric in sync when possible
        try {
          const pct = Math.round(bat.level * 100);
  ...
  function batterySnapshot() {
    if (cachedBattery) {
      return {
        pct: Math.round(cachedBattery.level * 100),
        charging: !!cachedBattery.charging,
        source: 'device'
      };
    }
    // Fallback simulated until API resolves
    return { pct: 87, charging: false, source: 'sim' };
  }
  ...
  function buildPowerHolo() {
    ...
    const estHours = pct > 5 ? ((pct / 100) * (bat.charging ? 0.4 : 4.5)).toFixed(1) : '—';
    const html = [
      '<div class="holo-banner"><span class="holo-banner-dot"></span><span class="holo-banner-text">POWER MATRIX · ' + statusLabel + '</span></div>',
      holoRow('Primary Cell', pct + '%', { valueClass: statusCls, meter: pct, meterClass: meterCls }),
      holoRow('Charge State', bat.charging ? 'CHARGING' : 'ON BATTERY', { valueClass: bat.charging ? 'ok' : 'cyan' }),
      holoRow('Telemetry Source', bat.source === 'device' ? 'Device Battery API' : 'Estimated (API unavailable)', { valueClass: 'cyan' }),
      ...
      '<div class="holo-note">Power readings reflect your device battery when the browser exposes the Battery Status API (Chrome / Edge on supported platforms). Keep the system charged before extended sessions, Epidexios.</div>'
    ].join('');
    showHologram({
      title: 'POWER LEVELS',
      subtitle: 'Energy matrix · Live cell readout',
      icon: '⚡',
      theme: 'power',
      html: html,
      ttl: 16000
      ...
      recognition.onresult = (event) => {
        try {
          ...
          for (let i = event.resultIndex; i < event.results.length; i++) {
            ...
            // Pick highest-confidence alternative when available
            let best = r[0];
          ...
          if (isFinal) {
            // Stop listening UI immediately so TTS can take the mic
            isListening = false;
            updateListeningUI(false);
            try { recognition.stop(); } catch (_) {}
  ...
  function toggleListening() {
    ...
    const beginListen = () => {
      ...
      // Gentle cue — does not block recognition
